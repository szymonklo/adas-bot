...
def draw_lane_and_plates(plates_positions, lane_borders, image, plate_y, plate_x):
    ...
    for i, (lane_border_down, lane_border_up) in enumerate(zip(lane_borders[:-1], lane_borders[1:])):
        if lane_border_down is not None and lane_border_up is not None:
            ...
            image = cv2.line(image, (lane_border_down[0], bottom - i * height_step), (lane_border_up[0], bottom - (i + 1) * height_step), (255, 0, 0), 2)
            image = cv2.line(image, (lane_border_down[1], bottom - i * height_step), (lane_border_up[1], bottom - (i + 1) * height_step), (255, 0, 0), 2)
